...
def plotFluorescence( path, worm, ax1, ax2 ):
	...
	imgs = [ load_stack(fName)[0] for fName in flist ]
	...
	for img in imgs:
		...
		norm.append( np.sum(imgCrop*global_otsu) / np.sum(global_otsu) )

	norm = np.array(norm)/np.mean(norm)
	print(norm)
	...
	for idx, p in enumerate( pos ):
		...
		for jdx, img in enumerate( imgs ):
			...
			movie.append( np.hstack( ( imgCrop, global_otsu*np.max(imgCrop) ) ) )
			...
			data.append( np.sum(imgCrop*global_otsu) / ( np.sum(global_otsu) * norm[jdx] ) )
		...
		# imsave( os.path.join( path, worm + '_pos'+str(idx)+'_mask.tif' ), (np.array(movieMask)*(2**16-1)).astype(np.uint16) )
...
if __name__ == '__main__':
	...
	path = 'W:\\Nicola\\160704_cocoaBeadsTests'
	worm = 'C01'
