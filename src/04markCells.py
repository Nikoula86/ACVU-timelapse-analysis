...
class GUI(QtGui.QWidget):
    ...
    def initUI(self):
        ...
        actn1pp.triggered.connect( lambda item = '1.pp': self.actionFunct1pp('1.pp') )

        actn1ppp = QtGui.QAction('1.ppp', self.cellNamesBox)
        self.cellNamesBox.addAction(actn1ppp)
        actn1ppp.triggered.connect( lambda item = '1.ppp': self.actionFunct1ppp('1.ppp') )

        actn1ppa = QtGui.QAction('1.ppa', self.cellNamesBox)
        self.cellNamesBox.addAction(actn1ppa)
        actn1ppa.triggered.connect( lambda item = '1.ppa':  self.actionFunct1ppa('1.ppa') )
        ...
        actn4aa.triggered.connect( lambda item = '4.aa': self.actionFunct4aa('4.aa') )

        actn4aaa = QtGui.QAction('4.aaa', self.cellNamesBox)
        self.cellNamesBox.addAction(actn4aaa)
        actn4aaa.triggered.connect( lambda item = '4.aaa': self.actionFunct4aaa('4.aaa') )

        actn4aap = QtGui.QAction('4.aap', self.cellNamesBox)
        self.cellNamesBox.addAction(actn4aap)
        actn4aap.triggered.connect( lambda item = '4.aap': self.actionFunct4aap('4.aap') )
        ...
        self.cellNamesBox.connect(self.cellNamesBox, QtCore.SIGNAL("customContextMenuRequested(QPoint)" ), self.leftClicked)
    ...
    def eventFilter(self, widget, event):
        ...
        if widget == self.canvas1 and isinstance(event, QtGui.QMouseEvent) and event.buttons() & QtCore.Qt.LeftButton:
            self.leftClicked(event.pos())
            return True
        if widget == self.canvas1 and isinstance(event, QtGui.QMouseEvent) and event.buttons() & QtCore.Qt.RightButton:
            self.rightClicked(event.pos())
            return True
        return False

    def leftClicked(self, QPos):
        refpos = np.array( [ int( QPos.x() / 600. * 512. ), int( QPos.y() / 600. * 512. ), self.sl.value() ] )  
        # print(refpos)
    ...
    def rightClicked(self, QPos):
        refpos = np.array( [ QPos.x() / 600. * 512., QPos.y() / 600. * 512., self.sl.value() ] )  
        # print(refpos)
    ...
    def selectWorm(self):
        ...
        self.setWindowTitle('Label cells - ' + self.pathDial.split("/")[-2] + ' - ' + self.pathDial.split("/")[-1][:3] )
    ...
    def updateCanvas1(self):
        ...
        for idx, cell in self.currentCells.iterrows():
            ...
            if cell.Z == self.sl.value():
                ...
                self.text1.append( self.ax1.text( cell.X, cell.Y + 18, cell.cname, color='orange', fontsize=10, alpha=.7,
                        rotation=0 ) )
                self.points1.append( self.ax1.plot( cell.X, cell.Y, 'o', color='orange', alpha = .7, mew = 0, ms=6 )[0] )
    ...
    def initializeCanvas2(self):
        ...
        self.points2, = self.ax2.plot( gonadPos[0], gonadPos[1], 'o', color='blue', ms=10, mew=0, alpha=.5, lw = 0 ) 
    ...
    def updateCanvas2(self):
        ...
        self.text2.set_text( '%.2f' % ( self.timesDF.ix[ self.timesDF.tidxRel == self.tp.value(), 'timesRel' ].values[0] - tpL2 ) )
