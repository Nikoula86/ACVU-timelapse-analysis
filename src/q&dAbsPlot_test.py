# -*- coding: utf-8 -*-
"""
Created on Tue Jan 16 18:06:13 2018
...
#		times = times - times[1][0]
	...
	z1 = data[ 1 + data[0].index('absValZ1') ]# - data[ 1 + data[0].index('absValb1') ]
	z4 = data[ 1 + data[0].index('absValZ4') ]# - data[ 1 + data[0].index('absValb4') ]
	# make lines connected
	if len( times[1] ) > 0:
		times[0] = np.append(times[0],times[1][0])
		times[1] = np.append(times[1],times[2][0])
		z1[0] = np.append(z1[0],z1[1][0])
		z1[1] = np.append(z1[1],z1[2][0])
		z4[0] = np.append(z4[0],z4[1][0])
		z4[1] = np.append(z4[1],z4[2][0])
	else:
		times[0] = np.append(times[0],times[2][0])
		z1[0] = np.append(z1[0],z1[2][0])
		z4[0] = np.append(z4[0],z4[2][0])
...
#		count+=1
	for idx, val in enumerate( zip(times,z1,z4) ):
#		if (tdiv1!=tdiv4) and (np.sign(ratio[2][-1])==-1):
		# plot mothers
		ax1.plot( val[0], val[1],
					'-', lw = .5, color = colors[idx], alpha = alpha )
		ax1.plot( val[0], val[2],
					'-', lw = .5, color = colors[idx], alpha = alpha )
	return count
