...
def plotFluorescence( path, worm, ax1, channel = '488nm', color = 'black' ):
	print( path, worm )
	...
	for idx, key in enumerate( ['1.pp','4.aa'] ):
		...
		# data = sis.ix[ ( sis.tidx >= tIdxMin ) & ( sis.tidx <= tIdxMax ) ]
		# databckg = bckg.ix[ ( bckg.tidx >= tIdxMin ) & ( bckg.tidx <= tIdxMax ) ]

		sis._488nm -= bckg._488nm
		sis.times -= tDiv
		# print(len(exp),len(times))
		# print(exp)
		# print(times)
		...
		exp = ( cell.ix[ cell.tidx == tIdxFinal, '_488nm' ] - bckg.ix[ bckg.tidx == tIdxFinal, '_488nm' ] ).values[0]
	...
	ax1.plot( -np.diff(XYdata[0]), -np.diff(XYdata[1])/np.sum(XYdata[1]), 'o', color = color )
	if path == 'X:\\Simone\\160930_lag2GFP+mCherry\\':
		txt = 'E1'+worm
	if path == 'X:\\Simone\\161002_lag2GFP+mCherry\\':
		txt = 'E2'+worm
	ax1.text( -np.diff(XYdata[0]), -np.diff(XYdata[1])/np.sum(XYdata[1]), txt )
...
if __name__ == '__main__':
	...
	for idx, worm in enumerate( worms ):
		...
		plotFluorescence( path, w, ax1, channel = '488nm')#, color = colors[idx] )
