...
class GUI(QtGui.QWidget):
	...
	def selectWorm(self):
		...
		self.setWindowTitle('Mark gonad - ' + self.pathDial.split("\\")[-2] + ' - ' + self.pathDial.split("\\")[-1][:3] )
	...
	def initializeCanvas1(self):
		...
		gonadPos = [np.nan,np.nan]
		self.pointsplot, = self.ax1.plot( gonadPos[0], gonadPos[1], 'o', color='w', ms=10, mew=0, alpha=.8, lw = 0 ) 
		
		# print gonad position
		gonadBox = np.array( [ [np.nan,np.nan,np.nan,np.nan], [np.nan,np.nan,np.nan,np.nan] ] )
		self.boxplot, = self.ax1.plot( gonadPos[0], gonadPos[1], '--', color='w', alpha=.8, lw = 2 ) 
		
		# print time
		# print(self.timesDF.ix[ self.timesDF.tidxRel == self.tp.value(), 'timesRel' ])
		self.textplot = self.ax1.text( 5, 20, '--.--', color = 'white', fontsize = 20 )     
	...
	def updateCanvas1(self):
		...
		if len( gonadPos.shape ) > 0:
			...
			self.boxplot.set_xdata( [gonadPos[0]-size/8.,gonadPos[0]+size/8.,gonadPos[0]+size/8.,gonadPos[0]-size/8.,gonadPos[0]-size/8.] )
			self.boxplot.set_ydata( [gonadPos[1]-size/8.,gonadPos[1]-size/8.,gonadPos[1]+size/8.,gonadPos[1]+size/8.,gonadPos[1]-size/8.] )
			plt.draw()
		...
		# print(self.timesDF.ix[ self.timesDF.tidxRel == self.tp.value(), 'timesRel' ])
		self.textplot.set_text( '%.2f' % ( self.timesDF.ix[ self.timesDF.tidxRel == self.tp.value(), 'timesRel' ].values[0] - tpL2 ) )
