...
def crop_image( imgs, c, size ):
    ...
    if len(imgs.shape) == 3:
        ...
        cropstack[ : , 
                    -int(np.min( [ c[1]-size/2, 0 ] )) : int(size-np.max( [ c[1]+size/2-dim[2]+1, 0 ] )) , 
                    -int(np.min( [ c[0]-size/2, 0 ] )) : int(size-np.max( [ c[0]+size/2-dim[1]+1, 0 ] )) ] = imgs[ :,
                    int(np.max( [ c[1]-size/2, 0 ] )) : int(np.min( [ c[1]+size/2, dim[2]-1 ] )) , 
                    int(np.max( [ c[0]-size/2, 0 ] )) : int(np.min( [ c[0]+size/2, dim[1]-1 ] )) ]
    if len(imgs.shape) == 2:
        cropstack = np.zeros( ( size, size ) )

        cropstack[
                    -int(np.min( [ c[1]-size/2, 0 ] )) : int(size-np.max( [ c[1]+size/2-dim[1]+1, 0 ] )) , 
                    -int(np.min( [ c[0]-size/2, 0 ] )) : int(size-np.max( [ c[0]+size/2-dim[0]+1, 0 ] )) ] = imgs[
                    int(np.max( [ c[1]-size/2, 0 ] )) : int(np.min( [ c[1]+size/2, dim[1]-1 ] )) , 
                    int(np.max( [ c[0]-size/2, 0 ] )) : int(np.min( [ c[0]+size/2, dim[0]-1 ] )) ]
...
def load_data_frame_pandas( path, fileName ):
...
def extract_current_cell_out( dict_pos, dict_out, tp, imgpxl=np.nan ):
    ...
    if len(cellsDF) > 0:
        ...
        for idx, row in cellsDF.iterrows():
            ...
            # cellsDF.area.values[idx] = dict_out[ row.cname ].ix[ dict_out[ row.cname ].tidx == tp, 'area' ].values[0]
...
def update_cell_out_DF( currentCells, cellOutDF, tp ):
    ...
    for cname in cellOutDF.keys():
        ...
        newCell.area.values[index] = np.nan
    ...
    for idx, cell in currentCells.iterrows():
        ...
        area = np.sum( p.contains_points(points).reshape(cell.imgPxl,cell.imgPxl).T )
        if area == 0:
            area = np.nan
        ...
        newCellOutDF[ cell.cname ].area.values[index] = [ area ]
...
def flat_field_correction( imgs, darkField, flatField, gp ):
    ...
    size = imgs[0].shape[0]
    medianCorrection = np.mean( ( flatField - darkField ) )
...
def extract_current_cell_fluo_qd( dict_pos, dict_fluo, tp ):
