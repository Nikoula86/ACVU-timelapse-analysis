...
class ACVU_simulation:
    ...
    def __init__(self,param_file):
        ...
        self.species_lbl=['O','Oc','OH','OcH','M','D']
        self.species_ind={'O':0,'Oc':1,'OH':2,'OcH':3,'M':4,'D':5}
    ...
    def calculate_propensity_basic(self,N,cell_type):
        ...
        for i in [0,1]:
            ...
            if i==0:
                S=phi * N[ self.S_ind('D',1) ]
            else:
                S=phi * N[ self.S_ind('D',0) ]
    ...
    def parallel_sweep( self, zipped_data ):
        ...
        N=self.initialize_reactants([{'Oc':1},{'Oc':1}])
        # and calculate stoichiometry matrix
#        dN=self.construct_dN()p
    ...
    def do_sweep(self,sweep_param_lbl,sweep_param_list,N_run,sweep_data_dir,sweep_data_file,save_species_list,seedStart=0):
...
#            new_run_data = [ None for i in range(num_new_sim) ]
            ...
            if os.cpu_count()>1:
                ...
                with Pool( int( os.cpu_count() * 1. ) ) as pool:
